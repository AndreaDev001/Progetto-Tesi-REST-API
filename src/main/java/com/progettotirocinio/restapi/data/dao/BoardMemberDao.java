...
public interface BoardMemberDao extends JpaRepository<BoardMember, UUID> {
    ...
    @Query("select b from BoardMember b where b.board.id = :requiredID and not exists (select t from TeamMember t where t.team.id = :requiredTeamID and t.member.user.id = b.user.id and t.team.board.id = :requiredID)")
